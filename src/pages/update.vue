<template>
    <!-- <Header /> -->
    <div class="w-full min-h-svh flex items-center justify-center bg-gradient-to-br from-gray-100 via-gray-200 to-gray-300 dark:from-slate-950 dark:via-slate-900 dark:to-slate-800 p-4 sm:p-6 md:p-8">
        <div class="w-full max-w-7xl">
            <div class="flex flex-col-reverse sm:flex-row gap-6 sm:gap-8">
                <div
                    ...
                    <label class="w-full p-6 border-2 border-dashed border-green-400 dark:border-green-700 rounded-lg bg-green-50 hover:bg-green-100 dark:bg-slate-800 dark:hover:bg-slate-700 transition-colors duration-300 cursor-pointer group">
                        <div class="flex flex-col justify-center items-center space-y-2 text-green-600 dark:text-green-400 group-hover:text-green-700 dark:group-hover:text-green-300">
                ...
                <Card class="w-full flex-1 shadow-lg bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm border-2 border-green-300 dark:border-green-800">
                    <CardHeader>
                        <CardTitle>
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                                <div class="text-3xl font-bold text-green-800 dark:text-green-300">修改宝贝</div>
                                <div class="text-sm font-medium text-green-600 dark:text-green-400">出售二手，减少碳足迹，为地球减负</div>
                            </div>
                        </CardTitle>
                        <CardContent>
                            <form class="space-y-6">
                             ...
                             <Button :disabled="isloading" class="mt-6 w-full bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-600 text-white font-semibold py-3 rounded-lg transition-colors duration-300" @click="updateProduct">
                                <span v-if="!isloading">修改</span> 
...
const updateProduct = async () => {
    if(product.value.title === ""|| product.value.price === '' || product.value.categoryId === '' || product.value.desc === "" || imgList.value.length === 0 || product.value.freight === '' ) {
        info("请填写完整信息")
        return
    }
    await uploadImg()
...
const loadData = async(id)=> {
    await api({
        ...
        if(res.code===1) {
            ...
            product.value.categoryId = res.data.categoryId.toString()
            imgList.value = new Array(res.data.image.length)
            fileList.value = new Array(res.data.image.length)
            const imagePromise = product.value.image.forEach((image, index) => {
...
const getImage =(url,index) => {
    return new Promise((resolve,reject) => {
        ...
        axios.get(`/img/${filename}`,{"responseType": 'blob'}).then(rst=>{
            const compressedFile = new File([rst.data], filename, { type: rst.data.type });
            imgList.value[index] = {src: url}
            console.log(filename)
            fileList.value[index] = compressedFile
            resolve()
        }).catch(err=>{
